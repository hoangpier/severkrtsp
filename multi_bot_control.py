# PHIÊN BẢN NÂNG CẤP - HỖ TRỢ NHIỀU TÀI KHOẢN CHÍNH
import discum
import threading
import time
import os
...
# --- CẤU HÌNH ---
# THAY ĐỔI: Sử dụng một danh sách cho các token chính
...
# THAY ĐỔI: Quản lý trạng thái các bot chính bằng một danh sách các dictionary
main_bot_states = []
spam_enabled = False
...
def save_settings():
    ...
    # Chỉ lưu các thông tin cần thiết, không lưu instance của bot
    savable_main_bot_states = [
        {'enabled': state['enabled'], 'threshold': state['threshold']}
        for state in main_bot_states
    ]

    settings = {
        'main_bot_states': savable_main_bot_states,
        'spam_enabled': spam_enabled, 'spam_message': spam_message, 'spam_delay': spam_delay,
        'bot_active_states': bot_active_states, # Trạng thái acc phụ
...
def load_settings():
    ...
    global bot_active_states, grab_channel_ids, spam_channel_id, check_channel_id
    ...
    try:
        ...
        if req.status_code == 200:
            ...
            if settings:
                ...
                spam_channel_id = settings.get('spam_channel_id', None)
                check_channel_id = settings.get('check_channel_id', None)

                # Cập nhật trạng thái cho các bot chính
...
def create_bot(token, bot_index=-1):
    """Tạo một instance bot. bot_index >= 0 nếu là bot chính."""
    ...
    def on_ready(resp):
        if resp.event.ready:
            ...
            if user_id:
                bot_type = f"(NODE {bot_index + 1})" if bot_index >= 0 else "(Sub)"
    ...
    if bot_index >= 0:
        ...
        # Delay config cho các bot chính, lặp lại mẫu nếu có nhiều hơn 3 bot
        delay_configs = [
        ...
        def create_grab_handler(bot_instance, state_dict, bot_name, reaction_conf):
            def on_message(resp):
                ...
                channel_id_of_drop = msg.get("channel_id")
                is_valid_drop = (
                    msg.get("author", {}).get("id") == karuta_id and
                    channel_id_of_drop in grab_channel_ids and
                    "is dropping" not in msg.get("content", "") and
                    not msg.get("mentions", []) and
                    state_dict['enabled']
                )
                if not is_valid_drop: return
                    ...
                    try:
                        ...
                        for msg_item in messages:
                            if msg_item.get("author", {}).get("id") == heart_bot_id and "embeds" in msg_item and msg_item["embeds"]:
                                ...
                                if sum(heart_numbers) > 0 and max_num >= state_dict['threshold']:
                                    max_index = heart_numbers.index(max_num)
                                    emoji, delay = reaction_conf[max_index]
                                    print(f"[{bot_name}] Chọn dòng {max_index+1} ({max_num} tim) -> Grab sau {delay}s tại kênh {channel_id_of_drop}", flush=True)
                                    ...
                                    def grab_and_check():
                                        try:
                                            ...
                                            if check_channel_id and check_channel_id.isdigit():
                                                bot_instance.sendMessage(check_channel_id, "kt b")
                                                print(f"[{bot_name}] Đã gửi 'kt b' đến kênh check {check_channel_id}", flush=True)
                                        except Exception as e:
                                            print(f"Lỗi khi grab và check ({bot_name}): {e}", flush=True)
    ...
    while True:
        try:
            if spam_enabled and spam_message and spam_channel_id and (time.time() - last_spam_time) >= spam_delay:
                ...
                print(f"Bắt đầu chu kỳ spam với {len(bots_to_spam)} bots...", flush=True)
                for idx, bot in enumerate(bots_to_spam):
                    ...
                    try:
                        ...
                        print(f"[{acc_name}] đã spam: {spam_message} tới kênh {spam_channel_id}", flush=True)
...
<head>
    ...
    <style>
        :root { --primary-bg: #0a0a0a; --panel-bg: #111111; --border-color: #333333; --blood-red: #8b0000; --necro-green: #228b22; --shadow-cyan: #008b8b; --text-primary: #f0f0f0; --text-secondary: #cccccc; }
        body { font-family: 'Courier Prime', monospace; background: var(--primary-bg); color: var(--text-primary); margin: 0; padding: 20px; }
        .container { max-width: 1200px; margin: 0 auto; }
        h1, h2 { font-family: 'Nosifer', cursive; text-align: center; color: var(--blood-red); }
        ...
        .grab-section, .spam-section { margin-bottom: 20px; padding: 15px; background: rgba(0,0,0,0.2); border: 1px solid var(--border-color); border-radius: 8px; }
        ...
        .status-badge { padding: 4px 10px; border-radius: 15px; font-size: 0.8em; }
    ...
    <div class="container">
        ...
        <div class="main-grid">
            <div class="panel">
                ...
                <div id="harvest-nodes-container"></div>
                <hr class="divider">
            ...
            <div class="panel">
                ...
                <div class="spam-section">
                    <h4>AUTO SPAM <span id="spam-status-badge" class="status-badge"></span></h4>
                    <div class="input-group"><input type="text" id="spam-channel-id" placeholder="ID Kênh Spam..."></div>
                    <div class="input-group"><textarea id="spam-message" rows="2" placeholder="Nội dung spam..."></textarea></div>
                    <div class="input-group"><input type="number" id="spam-delay" placeholder="Delay (s)"></div>
                    <button type="button" id="spam-toggle-btn" class="btn"></button>
                </div>
            </div>
            <div class="panel">
                 <h3><i class="fas fa-server"></i> Kênh Nhặt Thẻ</h3>
                 <div class="input-group">
                     <input type="text" id="new-channel-id" placeholder="Nhập ID Kênh Nhặt Thẻ...">
                     <button type="button" id="add-channel-btn" class="btn btn-necro" style="width: auto;">Thêm</button>
                 </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        async function postData(url = '', data = {}) {
            try {
                ...
                showStatusMessage('Lỗi giao tiếp với server.');
            }
        }
        ...
        function updateElement(id, { textContent, className, value }) {
            ...
            if (value !== undefined && document.activeElement.id !== id) {
                 el.value = value;
            }
        }
        ...
        async function fetchStatus() {
            try {
                ...
                // THAY ĐỔI: Tạo động các control cho harvest nodes
                const harvestContainer = document.getElementById('harvest-nodes-container');
                harvestContainer.innerHTML = ''; // Xóa control cũ
                data.main_bot_states.forEach((state, index) => {
                    const nodeDiv = document.createElement('div');
                    nodeDiv.className = 'grab-section';
                    
                    const statusText = state.enabled ? 'ON' : 'OFF';
                    const statusClass = state.enabled ? 'active' : 'inactive';
                    const actionText = state.enabled ? 'DISABLE' : 'ENABLE';
                    const buttonClass = state.enabled ? 'btn-blood' : 'btn-necro';
                    
                    nodeDiv.innerHTML = `
                        <h4>NODE ${index + 1} <span class="status-badge ${statusClass}">${statusText}</span></h4>
                        <div class="input-group">
                            <input type="number" id="heart-threshold-${index}" value="${state.threshold}" min="0">
                            <button type="button" data-node-index="${index}" class="btn ${buttonClass} harvest-toggle-btn">${actionText}</button>
                        </div>
                    `;
                ...
                updateElement('spam-channel-id', { value: data.spam_channel_id });
        ...
        setInterval(fetchStatus, 3000); // Lấy status 3 giây một lần
        fetchStatus();

        // --- Event Listeners ---
        // THAY ĐỔI: Dùng event delegation cho các nút harvest
        document.getElementById('harvest-nodes-container').addEventListener('click', e => {
            if (e.target.classList.contains('harvest-toggle-btn')) {
                const nodeIndex = e.target.dataset.nodeIndex;
                const threshold = document.getElementById(`heart-threshold-${nodeIndex}`).value;
                postData('/api/harvest_toggle', { node_index: nodeIndex, threshold: threshold });
            }
        });
        
        document.getElementById('spam-toggle-btn').addEventListener('click', () => postData('/api/spam_toggle', { 
            channel_id: document.getElementById('spam-channel-id').value,
            message: document.getElementById('spam-message').value, 
            delay: document.getElementById('spam-delay').value 
        }));
        
        document.getElementById('save-check-channel-btn').addEventListener('click', () => {
            const channelId = document.getElementById('check-channel-id').value;
            postData('/api/save_check_channel', { channel_id: channelId });
        });
        ...
        document.getElementById('add-channel-btn').addEventListener('click', () => {
            ...
            if (channelId) {
                postData('/api/add_grab_channel', { channel_id: channelId });
...
@app.route("/status")
def status():
    # Chuẩn bị dữ liệu trạng thái để gửi cho giao diện
    savable_main_bot_states = [
        {'enabled': state['enabled'], 'threshold': state['threshold']}
        for state in main_bot_states
    ]
    ui_states = {
        ...
        "spam_button_class": "btn-blood" if spam_enabled else "btn-necro",
    }
    return jsonify({
        'main_bot_states': savable_main_bot_states,
...
def api_harvest_toggle():
    # THAY ĐỔI: Xử lý theo index của node
    data = request.get_json()
    node_index = int(data.get('node_index'))
    threshold = int(data.get('threshold', 50))
    
    if 0 <= node_index < len(main_bot_states):
        state = main_bot_states[node_index]
        state['enabled'] = not state['enabled']
        state['threshold'] = threshold
        save_settings()
        msg = f"Node {node_index + 1} đã {'BẬT' if state['enabled'] else 'TẮT'}"
        return jsonify({'status': 'success', 'message': msg})
    return jsonify({'status': 'error', 'message': 'Node không hợp lệ'})

@app.route("/api/spam_toggle", methods=['POST'])
def api_spam_toggle():
    global spam_enabled, spam_message, spam_delay, spam_thread, last_spam_time, spam_channel_id
    data = request.get_json()
    spam_message = data.get("message", "").strip()
    spam_delay = int(data.get("delay", 10))
    new_spam_channel_id = data.get("channel_id", "").strip()
    
    if new_spam_channel_id and new_spam_channel_id.isdigit():
        spam_channel_id = new_spam_channel_id

    if not spam_enabled and spam_message and spam_channel_id:
        spam_enabled = True
        last_spam_time = time.time()
        msg = "Auto Spam ĐÃ BẬT."
        if spam_thread is None or not spam_thread.is_alive():
            spam_thread = threading.Thread(target=spam_loop, daemon=True)
            spam_thread.start()
    else:
        spam_enabled = False
        msg = "Auto Spam ĐÃ TẮT."
    save_settings()
    return jsonify({'status': 'success', 'message': msg})

@app.route("/api/save_check_channel", methods=['POST'])
def api_save_check_channel():
    global check_channel_id
    data = request.get_json()
    new_id = data.get('channel_id', '').strip()
    if new_id and new_id.isdigit():
        check_channel_id = new_id
        save_settings()
        return jsonify({'status': 'success', 'message': f'Đã lưu kênh check: {new_id}'})
    elif not new_id:
        check_channel_id = None
        save_settings()
        return jsonify({'status': 'success', 'message': 'Đã xóa kênh check.'})
...
def api_add_grab_channel():
    global grab_channel_ids
    channel_id = request.get_json().get('channel_id')
    if channel_id and channel_id.isdigit():
        if channel_id not in grab_channel_ids:
            grab_channel_ids.append(channel_id)
            save_settings()
            return jsonify({'status': 'success', 'message': f'Đã thêm kênh {channel_id}.'})
        else:
            return jsonify({'status': 'error', 'message': f'Kênh {channel_id} đã tồn tại.'})
    return jsonify({'status': 'error', 'message': 'ID kênh không hợp lệ.'})

# --- MAIN EXECUTION ---
if __name__ == "__main__":
    # Khởi tạo trạng thái cho các bot chính dựa trên .env
    for i, token in enumerate(main_tokens):
        if token.strip():
            main_bot_states.append({
                'token': token.strip(),
                'bot_instance': None,
                'enabled': False,
                'threshold': 50
    ...
    load_settings() # Tải cài đặt đã lưu, ghi đè trạng thái mặc định
    grab_channel_ids = [cid for cid in grab_channel_ids if cid]
    print("--- KHỞI TẠO BOT ---", flush=True)

    # Tạo instance cho các bot chính
    for i, state in enumerate(main_bot_states):
        state['bot_instance'] = create_bot(state['token'], bot_index=i)

    # Tạo instance cho các bot phụ
